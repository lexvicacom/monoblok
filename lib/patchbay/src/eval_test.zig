...
test "dropout emits lost on silence and found only after trip" {
    var arena_state: std.heap.ArenaAllocator = .init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    const slot = loaded[0].state.getPtr("dropout/n:device.a").?;
    try builtins.fireClocked(&fire_ctx, &loaded[0], slot, 100);
    try testing.expectEqual(@as(usize, 0), tp.buf.items.len);
    try builtins.fireClocked(&fire_ctx, &loaded[0], slot, 150);
    try testing.expectEqual(@as(usize, 1), tp.buf.items.len);
    try testing.expectEqualStrings("device.a.stale", tp.buf.items[0].subject);
    try testing.expectEqualStrings("true", tp.buf.items[0].payload);

    ctx.now_ms = 160;
    try run(loaded, &ctx);
    try testing.expectEqual(@as(usize, 2), tp.buf.items.len);
    try testing.expectEqualStrings("device.a.stale", tp.buf.items[1].subject);
    try testing.expectEqualStrings("false", tp.buf.items[1].payload);
