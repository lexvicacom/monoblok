...
MB_2=$(run_pub 2 5000000 64);    sleep "$COOLDOWN_S"
...
if $HAS_NATS_SERVER; then
    ...
    if kill -0 $NS_PID 2>/dev/null; then
        ...
        NS_2=$(run_pub 2 5000000 64);    sleep "$COOLDOWN_S"
...
if [ -n "$NS_1" ]; then
    ...
    printf "%-22s %14s %14s %8s\n" "--------" "--------" "-----------" "---"
    print_row "1 pub × 5M × 64B"    "$MB_1" "$NS_1"
    print_row "2 pub × 5M × 64B"    "$MB_2" "$NS_2"
    print_row "8 pub × 1M × 128B"   "$MB_3" "$NS_3"
    print_row "1 pub → 1 sub × 2M"  "$MB_4" "$NS_4"
    print_row "1 pub → 10 subs × 500k" "$MB_5" "$NS_5"
    print_row "1 pub → 50 subs × 200k" "$MB_6" "$NS_6"
else
    # monoblok only
    printf "%-22s %14s\n" "Workload" "monoblok"
    printf "%-22s %14s\n" "--------" "--------"
    print_row "1 pub × 5M × 64B"    "$MB_1" ""
    print_row "2 pub × 5M × 64B"    "$MB_2" ""
    print_row "8 pub × 1M × 128B"   "$MB_3" ""
    print_row "1 pub → 1 sub × 2M"  "$MB_4" ""
    print_row "1 pub → 10 subs × 500k" "$MB_5" ""
    print_row "1 pub → 50 subs × 200k" "$MB_6" ""
    echo
