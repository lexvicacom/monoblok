#!/usr/bin/env bash
# Benchmark the C spike against nats-server using the nats CLI.
#
# This is intentionally a smaller copy of ../../scripts/bench-with-nats-server.sh:
# the C spike implements only CONNECT/PING/PUB/SUB and literal fanout, so this
# measures core parser/router/write behavior rather than Monoblok feature parity.
set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$HERE/.." && pwd)"
MB_BIN="$ROOT/build/monoblok-c"
...
if [ ! -x "$MB_BIN" ]; then
    echo "building C spike..."
    cmake -S "$ROOT" -B "$ROOT/build" -DCMAKE_BUILD_TYPE=Release >/dev/null || exit 1
    cmake --build "$ROOT/build" --target monoblok-c >/dev/null || exit 1
fi
...
run_fanout() {
    local subs="$1" msgs="$2"
    local out="/tmp/monoblok-c-bench-sub.out"
...
echo
echo "monoblok-c $("$MB_BIN" --help 2>&1 | head -1 || true)"
echo "nats cli $(nats --version 2>&1 | head -1)"
$HAS_NATS_SERVER && echo "$(nats-server --version 2>&1 | head -1)"
echo

"$MB_BIN" --port "$PORT" > /tmp/monoblok-c-bench.log 2>&1 &
MB_PID=$!
sleep 0.3
kill -0 "$MB_PID" 2>/dev/null || {
    echo "monoblok-c failed to start:"
    cat /tmp/monoblok-c-bench.log
    exit 1
}

echo "Running monoblok-c benchmarks..."
...
if $HAS_NATS_SERVER; then
    nats-server --port "$PORT" > /tmp/monoblok-c-nats-server.log 2>&1 &
    NS_PID=$!
    sleep 0.4
    if kill -0 "$NS_PID" 2>/dev/null; then
        ...
        cat /tmp/monoblok-c-nats-server.log
    fi
fi
...
echo
if [ -n "$NS_1" ]; then
    printf "%-24s %14s %14s %8s\n" "Workload" "monoblok-c" "nats-server" "delta"
    printf "%-24s %14s %14s %8s\n" "--------" "----------" "-----------" "-----"
